...
const ReviewsPage = () => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(
        (item) =>
          ...
          (item.created_at && formatDate(item.created_at).toLowerCase().includes(term)) ||
          // Search by SR (sequence number in the table)
          (indexOfFirstEntry + reviewsData.indexOf(item) + 1).toString().includes(term)
      );
    }
  ...
  const handleSort = (key) => {
    ...
    const sortedData = [...filteredData].sort((a, b) => {
      // Handle numeric sorting for rating and ID
      ...
      // Handle text sorting for name and comment
      if (a[key] < b[key]) {
...
<div className="bg-white p-4 rounded-lg shadow-md mb-6 border border-light-accent">
  <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 flex-1">
      ...
      {/* Rating Filter + Date Range in same row */}
      <div className="relative flex flex-col sm:flex-row items-center w-full">
        ...
        {/* Date range, aligned like your cases example */}
                ...
                <table className="min-w-full divide-y divide-light-accent">
                  <thead className="bg-primary">
                    <tr>
                      <th
                        scope="col"
                        className="px-4 py-3 text-left text-xs font-medium text-light uppercase tracking-wider cursor-pointer w-16"
                      ...
                      <th
                        scope="col"
                        className="px-4 py-3 text-left text-xs font-medium text-light uppercase tracking-wider cursor-pointer"
                      ...
                      <th
                        scope="col"
                        className="px-4 py-3 text-left text-xs font-medium text-light uppercase tracking-wider cursor-pointer w-24"
                      ...
                      <th
                        scope="col"
                        className="px-4 py-3 text-left text-xs font-medium text-light uppercase tracking-wider cursor-pointer w-32"
                  ...
                  <tbody className="bg-white divide-y divide-light-accent">
                    {currentEntries.length > 0 ? (
                      currentEntries.map((review, index) => (
                        <tr key={review.id} className="hover:bg-light">
                          <td className="px-4 py-4 whitespace-nowrap text-sm text-dark w-16">
                            {indexOfFirstEntry + index + 1}
                          </td>
                          <td className="px-4 py-4 whitespace-nowrap text-sm font-medium text-dark">
                            {review.name}
                          </td>
                          <td className="px-4 py-4 whitespace-nowrap text-sm text-dark w-24">
                            {formatDate(review.created_at)}
                          </td>
                          <td className="px-4 py-4 whitespace-nowrap text-sm text-dark w-32">
                            {renderStars(review.rating)}
                          </td>
