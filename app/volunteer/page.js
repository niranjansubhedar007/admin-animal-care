...
//   }, [volunteerData, searchTerm, startDate, endDate, locationFilter]);
...
//     ...new Set(volunteerData.map((item) => item.location)),
//   ].filter(Boolean);
...
const VolunteerList = () => {
  ...
  const filterData = () => {
    ...
    // Apply search filter - including SR number
    if (searchTerm) {
      const term = searchTerm.toLowerCase();
      result = result.filter(
        (item, index) =>
          ...
          (item.address && item.address.toLowerCase().includes(term)) ||
          (item.location && item.location.toLowerCase().includes(term)) ||
          // Search by SR (sequence number in the table)
  ...
  const handleSort = (key) => {
    ...
    const sortedData = [...filteredData].sort((a, b) => {
      ...
      // Handle text sorting for other fields
      if (a[key] < b[key]) {
  ...
  // Get unique locations for filter dropdown - with Thane and Mumbai as options
  const uniqueLocations = [
    "Thane", 
    "Mumbai",
    ...new Set(volunteerData.map((item) => item.location).filter(loc => loc && loc !== "Thane" && loc !== "Mumbai"))
  ].filter(Boolean);
  ...
  <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 flex-1">
      ...
      {/* Location Filter + Date Range in same row */}
      <div className="relative flex flex-col sm:flex-row items-center w-full">
        <div className="absolute top-3 left-0 pl-3 flex items-center pointer-events-none">
          <MapPin className="h-5 w-5 text-primary" />
        </div>
        <select
          className="pl-10 block lg:w-40 md:w-40 w-full rounded-md border-light-accent shadow-sm focus:border-primary focus:ring-primary py-2 border appearance-none bg-white text-dark"
          value={locationFilter}
          onChange={(e) => setLocationFilter(e.target.value)}
        >
          <option value="all">All Locations</option>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-light-accent">
                  <thead className="bg-primary">
                    <tr>
                      <th 
                        className="px-6 py-3 text-left text-xs font-medium text-light uppercase tracking-wider cursor-pointer w-16"
                      ...
                      <th 
                        className="px-6 py-3 text-left text-xs font-medium text-light uppercase tracking-wider cursor-pointer w-28"
                      ...
                      <th 
                        className="px-6 py-3 text-left text-xs font-medium text-light uppercase tracking-wider cursor-pointer w-32"
                  ...
                  <tbody className="bg-white divide-y divide-light-accent">
                    {currentEntries.length > 0 ? (
                      currentEntries.map((item, index) => (
                        <tr key={item.id} className="hover:bg-light">
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-dark w-16">
                            {indexOfFirstEntry + index + 1}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-dark w-28">
                            {formatDate(item.created_at)}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-dark">
                            <div className="flex items-center">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-dark w-32">
                            <div className="flex items-center">
                              {item.mobile}
                            </div>
                          </td>
